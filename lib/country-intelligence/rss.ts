...
import { matchesNewsCountry } from "./news-countries";

// RSS from a fixed publisher allowlist only. No entity expansion, scripts,
// remote XML resources or arbitrary URL fetching.
export function parseNewsRss(xml: string, code: string, provider: "Anadolu Ajansı" | "BBC", now = new Date(), countryScope?: string): NewsItem[] {
  ...
  for (const block of xml.matchAll(/<item(?:\s[^>]*)?>([\s\S]*?)<\/item>/gi)) {
    ...
    if (!title || !url || (countryScope !== code && !matchesNewsCountry(title+" "+tag(block[1],"description"), code)) || !Number.isFinite(timestamp) || age < 0 || age > 7*86400000) continue;
    const publisherHost = provider === "Anadolu Ajansı" ? /^(www\.)?aa\.com\.tr$/ : /^(www\.)?bbc\.(com|co\.uk)$/;
    if (!publisherHost.test(new URL(url).hostname)) continue;
