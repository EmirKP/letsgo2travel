...
export type AirportOption = {
  id: string;
  type: "country" | "city" | "airport" | "anywhere";
  code: string;
  displayCode: string;
  city: string;
  name: string;
...
export const AIRPORT_OPTIONS: AirportOption[] = GLOBAL_LOCATIONS.map((loc) => ({
  id: loc.id,
  type: loc.type as any,
  code: loc.code,
  displayCode: loc.code,
  city: loc.type === "city" ? loc.name : (loc.type === "country" ? loc.name : ""),
  name: loc.type === "city" ? "" : loc.name,
  country: loc.countryName || loc.name,
...
export function airportLabel(option: AirportOption) {
  if (option.type === "country") return `${option.city || option.name} (${option.displayCode})`;
