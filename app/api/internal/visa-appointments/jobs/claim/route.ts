...
export async function POST(request: Request) {
  ...
  const { data, error } = await supabase.rpc("claim_visa_appointment_jobs", {
    p_worker_name: workerName,
    p_limit: limit,
  });
  if (error) return NextResponse.json({ error: "Görevler alınamadı.", detail: error.message }, { status: 500 });
