...
export async function POST(request: Request) {
  ...
  try {
    ...
    // Kanal tercihi: varsayilan e-posta. Telefon bildirimi yalniz giris yapmis
    // kullanicilar icin secilebilir (cihaz tokenlari hesaba baglidir) ve
    // varsayilan olarak KAPALIDIR.
    const wantsEmail = notifyEmail === undefined ? true : Boolean(notifyEmail);
    const wantsPush = Boolean(notifyPush) && Boolean(requestUser.user);
    if (!wantsEmail && !wantsPush) {
      return NextResponse.json({ error: "En az bir bildirim kanalı seçmelisin (e-posta veya telefon bildirimi)." }, { status: 400 });
    }
    if (Boolean(notifyPush) && !requestUser.user) {
      return NextResponse.json({ error: "Telefon bildirimi için giriş yapman gerekiyor. E-posta kanalıyla devam edebilirsin." }, { status: 401 });
    }
    ...
    // Onay e-postasi YALNIZ e-posta kanali secildiyse gonderilir.
    // Push-only kullaniciya alarm olusturma e-postasi GONDERILMEZ.
    let confirmationMailOk: boolean | null = null;
    if (wantsEmail) {
      ...
      confirmationMailOk = Boolean(mailRes.success) && mailRes.providerId !== "mock";

      if (!confirmationMailOk && !wantsPush) {
        // Yalniz e-posta secilmis ve onay e-postasi gonderilememis:
        // alarm guvenli sekilde geri alinir.
      ...
      // created_sent YALNIZ e-posta gercekten gonderildiyse yazilir.
      await supabase.from("flight_price_alerts").update({
        last_mail_status: confirmationMailOk ? "created_sent" : "created_failed",
        last_error_message: confirmationMailOk ? null : "Onay e-postası gönderilemedi.",
        last_error_at: confirmationMailOk ? null : new Date().toISOString(),
      }).eq("id", data.id);
    }

    // Kullaniciya donen mesaj GERCEKTEN etkin kanallari soyler.
    let message: string;
    if (wantsEmail && wantsPush) {
      ...
      message = "Fiyat alarmın kuruldu. Fiyat düşünce telefon bildirimi alacaksın (e-posta bildirimi kapalı).";
    }
