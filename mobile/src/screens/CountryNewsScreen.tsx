  ...
  return <div className="screen country-news-screen ci-screen">
    ...
    <div className="ci-content">
      ...
      {data && <>
        ...
        {(filter === "all" || filter === "calendar") && <section className="ci-section"><h2>{copy("Önümüzdeki 30 gün", "The next 30 days")}</h2>
          {data.calendar.map(item => <article className="ci-calendar-row" key={item.id}><div className="ci-date-tile"><strong>{item.date.slice(8)}</strong><small>{new Date(`${item.date}T12:00:00Z`).toLocaleDateString(locale, { month: "short", timeZone: "UTC" })}</small></div><div><span className="ci-eyebrow">{item.date === data.today ? `${copy("Bugün", "Today")} · ` : ""}{item.type === "observance" ? copy("Anma günü · Resmî tatil değil", "Observance · Not a public holiday") : copy("Resmî tatil", "Public holiday")}</span><h3>{locale === "tr" ? item.name.tr : item.name.en}</h3><p>{item.type === "observance" ? copy("09.05'te anma törenleri yapılır. Sirenleri acil durumla karıştırma; çevredeki törenlere saygı göster.", "Remembrance ceremonies take place at 09:05. Do not confuse the sirens with an emergency; be mindful of local ceremonies.") : `${item.countryWide ? copy("Ülke geneli", "Nationwide") : `${copy("Bölgesel", "Regional")}${item.regions.length ? ` · ${item.regions.join(", ")}` : ""}`}. ${copy("Kurumların ve ulaşımın çalışma saatleri değişebilir.", "Institution and transport hours may change.")}`}</p><button type="button" className="ci-text-button" onClick={() => void openExternal(item.sourceUrl)}>{copy("Takvim kaynağı", "Calendar source")}<Icon name="external" size={13}/></button></div></article>)}
        ...
        {filter !== "calendar" && <section className="ci-section"><h2>{copy("Son haberler", "Recent news")}</h2><p className="ci-muted">{copy("Son 7 günde GDELT tarafından bulunan başlıklar. Bulunma tarihi, yayın veya olay tarihi değildir. Başlıklar kaynağın dilindedir.", "Headlines found by GDELT in the last 7 days. Discovery time is not the publication or event date. Headlines retain their source language.")}</p>
          {news.map(item => <article className="ci-news-card" key={item.url}><span className="ci-eyebrow">{item.publisher} · {copy("Bulundu", "Found")}: {date(item.firstSeenAt)}</span><h3><button type="button" onClick={() => void openExternal(item.url)}>{item.title}<Icon name="external" size={15}/></button></h3><div className="ci-impact"><strong>{copy("Seyahatine etkisi · Otomatik yorum", "Travel impact · Automated note")}</strong><p>{copy(...IMPACT[item.topic])}</p></div></article>)}
          {!news.length && <div className="ci-empty">{data.newsState === "ok" ? copy("Bu filtre için güncel haber bulunamadı.", "No recent headlines match this filter.") : copy("Haber akışı şu an yanıt vermiyor. Resmî uyarı ve takvim kaynakları yukarıda ayrı gösterilir.", "The news feed is unavailable. Official advice and calendar sources are shown separately above.")}</div>}
        </section>}
