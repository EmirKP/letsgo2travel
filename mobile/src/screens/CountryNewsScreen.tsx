...
import { prominentAdvisory } from "../../../lib/country-intelligence/advisory-reports";
...
export function CountryNewsScreen({ initialCountry = "TR" }: { initialCountry?: string }) {
  ...
  const advisory = data && prominentAdvisory(data.advisory);
  ...
  return <div className="screen country-news-screen ci-screen">
    ...
    <div className="ci-content">
      ...
      {data && <>
        ...
        {(filter === "all" || filter === "security") && <AdvisoryCard advisory={data.advisory}/>}
        ...
        {(filter === "all" || filter === "security") && advisory && advisory.updates.length > 0 && <details className="ci-method"><summary>{copy("Resmî duyuru geçmişi", "Official update history")} · {advisory.source.name}</summary><p>{copy("Kaynağın özgün dili ve gerçek güncelleme tarihiyle gösterilir.", "Shown in the source language with its actual update date.")}</p>{advisory.updates.map((update, index) => <p key={`${update.date}-${index}`}><strong>{date(update.date)}</strong><br/>{update.text}</p>)}</details>}
