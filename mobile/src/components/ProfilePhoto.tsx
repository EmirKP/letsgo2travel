...
export function ProfilePhoto({ userId, accessToken, name, onSignIn, onNotice }: {
  ...
  return <div className="profile-photo-control">
    <button type="button" className="profile-photo" onClick={() => userId ? input.current?.click() : onSignIn()} aria-label={copy("Profil fotoğrafı seç", "Choose profile photo")} disabled={busy}>
    ...
    <input ref={input} className="sr-only" type="file" accept="image/jpeg,image/png,image/webp,image/heic,image/heif" tabIndex={-1} onChange={event => {
      ...
      if (file) void preparePhoto(file).then(setPreview).catch(error => onNotice(error.message === "size" ? copy("20 MB'den küçük bir fotoğraf seç.", "Choose a photo smaller than 20 MB.") : copy("Fotoğraf açılamadı. JPEG veya PNG olarak tekrar seç.", "Cannot open this photo. Try a JPEG or PNG.")));
    }}/>
    {preview && <div className="avatar-confirm" role="group" aria-label={copy("Fotoğrafı onayla", "Confirm photo")}>
      <img src={preview} alt={copy("Kaydedilecek fotoğraf", "Photo to save")}/>
